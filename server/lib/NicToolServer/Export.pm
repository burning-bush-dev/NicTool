...
sub set_status {
    my ( $self, $message ) = @_;
    $self->exec_query( "UPDATE nt_nameserver SET export_status=?  WHERE nt_nameserver_id=?",
        [ $message, $self->{ns_id} ] );
}
...
sub touch_publish_ts {
    ...
    $self->exec_query( "UPDATE nt_zone SET last_publish=NOW() WHERE zone=? AND deleted=?",
        [ $zone, $deleted ] );
}
...
sub get_last_ns_export {
    ...
    if ( $logs && scalar @$logs == 0 ) {
        ...
        $message = "no previous successful export"
            if defined $p{success} && $p{success} == 1;
        $self->elog($message);
        return;
    }
    return $logs->[0];
...
sub get_rr_types {
    ...
    $self->{rr_types} = $self->exec_query("SELECT id,name,description FROM resource_record_type");
  ...
  ORDER BY ns.nt_nameserver_id";
    ...
    if ( $self->{ns_id} ) {
        ...
        my $first = $self->{active_ns_ids}{ $self->{active_ns}[0] }
            || $self->{active_ns}[0];
        $self->{export_format} = $first->{export_format}
            if $first->{export_format};
    }
