...
sub get_user {
    ...
    if ( !$user_perm ) {
        return $self->error_response( 507,
            "Could not find permissions for user (" . $data->{nt_user_id} . ")" );
    }
...
sub edit_user {
    ...
    my @columns = grep { exists $data->{$_} } qw/ nt_group_id first_name last_name username email /;
    ...
    my %nonperm  = map  { $_ => 1 } grep { !$data->{user}{$_} } $self->perm_fields;
    my @permcols = grep { exists $data->{$_} && $data->{user}{$_} } $self->perm_fields;
    ...
    if ( !$prev_data->{inherit_group_permissions} ) {
        ...
        $sql =
            ...
            . join( ',', 'nt_group_id', 'nt_user_id', @permcols )
            . ") VALUES("
            . join( ',', 0, $data->{nt_user_id}, map( $dbh->quote( $data->{$_} ), @permcols ), )
            . ")";
...
sub get_group_users {
    ...
    my %field_map = (
        username   => { timefield => 0, quicksearch => 1, field => 'nt_user.username' },
        first_name => {
            timefield   => 0,
            quicksearch => 0,
        ...
        email => { timefield => 0, quicksearch => 0, field => 'nt_user.email' },
    );
    ...
    my $sortby     = $self->format_sort_conditions( $data, \%field_map, "nt_user.username" );
...
sub select_user {
    ...
    my $users = $self->exec_query( "SELECT * FROM nt_user WHERE nt_user_id = ?", $uid )
        or return {
        error_code => 600,
        error_msg  => $self->{dbh}->errstr,
        };
...
sub verify_ldap_user {
    ...
    if ($NicToolServer::ldap_filter) {
        ...
        return 0
            if ( $user_dn eq '' );    # Locating user failed. Return failed attempt
...
sub locate_ldap_user {
    ...
    if ( $ldap_result->code ) {
        warn 'LDAP: search failed: ' . $ldap_result->error
            if $self->debug_auth;
        return '';
    }
