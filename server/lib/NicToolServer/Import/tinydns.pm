...
sub import_records {
    ...
    while ( defined( my $record = <$fh> ) ) {
        ...
        if ( $first eq 'Z' ) {  #  SOA        =>  Z fqdn:mname:rname:ser:ref:ret:exp:min:ttl:time:lo
            ...
            push @err,
                $self->zr_ptr(
                join( ':', $self->ip_to_ptr($addr), $fqdn, $ttl || '', $ts || '', $loc || '' ) );
        }
            ...
            push @err,
                $self->zr_ptr(
                join( ':', $self->ip6_to_ptr($addr), $fqdn, $ttl || '', $ts || '', $loc || '' ) );
        }
...
sub zr_soa {
    ...
    print "SOA     : $r\n";
    my ($zone,   $mname, $rname, $serial,    $refresh, $retry,
        $expire, $min,   $ttl,   $timestamp, $location
    ) = split( ':', $r );
