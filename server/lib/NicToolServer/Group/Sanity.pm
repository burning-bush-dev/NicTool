...
sub new_group {
    ...
    $self->error( 'nt_group_id', 'Cannot add group to a deleted group!' )
    ...
    unless ( lc( $data->{name} ) =~ /^[a-z0-9]/ ) {
        $self->error( 'name', 'Group name must start with a letter or number. (A-Z, 0-9)' );
    }
...
sub edit_group {
    ...
    $self->error( 'nt_group_id', 'Cannot edit group in a deleted group!' )
        if $self->check_object_deleted( 'group', $dataobj->{parent_group_id} );
...
sub get_group_subgroups {
    ...
    $self->search_params_sanity_check( $data, qw(group sub_groups parent_group_id) );
    return $self->throw_sanity_error if $self->{errors};
...
sub get_global_application_log {
    ...
    $self->search_params_sanity_check( $data, qw(timestamp user action object title description) );
    return $self->throw_sanity_error if $self->{errors};
