...
sub zr_spf {
    ...
    my $rdata = join( '',
        map { $self->characterCount($_) . $self->escape($_) } unpack "(a255)*",
        $r->{address} );
...
sub zr_caa {
    ...
    $rdata .= $self->characterCount( $r->{other} ) . $self->escape( $r->{other} );
...
sub zr_srv {
    ...
    my $rdata =
          escapeNumber( $self->{nte}->is_ip_port( $r->{priority} ) )
...
sub zr_loc {
    ...
    my $rdata = octal_escape(
        pack 'C4N3', 0, precsize_valton($size),
...
sub zr_naptr {
    ...
    my $rdata = $self->escapeNumber( $r->{'weight'} )    # order, 16-bit
        ...
        . $self->characterCount($services)
        . $self->escape($services)
        . $self->characterCount($regexp)
        . $self->escape($regexp);
...
sub zr_rrsig {
    ...
    my ($type, $algorithm, $label_count, $orig_ttl,     $sig_exp,
        undef, $sig_inc,   $key_tag,     $signers_name, $signature
    ) = split /\s+/, $r->{address}, 10;

    $type = $self->{nte}->get_rr_id($type);    # convert from RR name to ID

    $signature =~ s/\s+//g;
    chop $signature;                           # remove spaces and trailing )
...
sub pack_type_bitmap {
    ...
    my ($highest_rr_id) =
        ( sort { $a <=> $b } keys %rec_ids )[-1];    # find highest ID
    ...
    foreach my $window ( 0 .. $highest_window ) {
        ...
        foreach my $octet ( 0 .. $bm_octets - 1 ) {
            my $start     = $octet * 8;
            my $bitstring = join '', map { defined $rec_ids{$_} ? 1 : 0 } $start .. $start + 7;
...
sub dms2latlon {
    ...
    $retval =
        ( $deg * $conv_deg ) + ( $min * $conv_min ) + ( $sec * $conv_sec );
