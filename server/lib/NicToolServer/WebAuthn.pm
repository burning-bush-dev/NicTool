...
sub _consume_challenge {
    ...
    my ( $sql, @bind );
    if ( defined $uid ) {
        $sql = 'SELECT * FROM nt_user_webauthn_challenge
          WHERE challenge = ?
            AND ceremony_type = ?
            AND nt_user_id = ?
            AND consumed = 0
    ...
    my $rows = $self->exec_query( $sql, \@bind );

    return if !$rows || !$rows->[0];

    $self->exec_query(
        'UPDATE nt_user_webauthn_challenge
            SET consumed = 1
          WHERE nt_webauthn_challenge_id = ?',
        $rows->[0]{nt_webauthn_challenge_id}
    );
